...
class ActivitySchedulingCoverageTest < ActionDispatch::IntegrationTest
  setup do
    @user = users(:one)
  ...
  test "activity_scheduling/show.html.erb renders without Google account" do
    # Remove Google account and activities to test different state
  ...
  test "activity_scheduling/preview.html.erb renders rescheduled events" do
    # This tests the rescheduled badge display
    sign_in @user
    ...
    # The rescheduled badge appears conditionally based on notes
    # We test that the template can render it even if no events are rescheduled
    assert_response :success
  end
